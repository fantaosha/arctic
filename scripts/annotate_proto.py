...
from mesh_fitting.optim.model.proto import Proto
from mesh_fitting.atlas.proto import Proto as OrigProto
...
def anno_frames_for_single_cam(
    ...
    atlas_dir = "/private/home/taoshaf/data/atlas/proto"

    data_dir: str = os.path.join(data_dir, seq)
    result_dir: str = os.path.join(result_dir, seq, "results")

    result_file = os.path.join(result_dir, "proto", keypoint, result_file)
    ...
    shape_params = torch.tensor(
        np.array([results[frame_idx]["shape_params"][:45] for frame_idx in frame_idxs]),
        device=device,
        dtype=dtype,
    )
    ...
    orig_hand_params = torch.cat(
        [opt_params[..., 114:146], opt_params[..., 82:114]], dim=-1
    )
    orig_scale_params = opt_params[:, -43:].clone()
...
def main():
    ...
    parser.add_argument(
        "--anno_dir",
        type=str,
        default="/private/home/taoshaf/data/annotation/arctic/annos/proto/full",
    )
    parser.add_argument(
    ...
    parser.add_argument("--keypoint", type=str, default="dense")
    ...
    proto_dir = "/private/home/taoshaf/data/atlas/proto"
